...
-- ║                                                                              ║
-- ║  Creates: Notifications table, Workforce Analytics views,                    ║
-- ║           HMS Integration helper views                                       ║
-- ║                                                                              ║
-- ║  SAFE TO RUN: Uses IF NOT EXISTS / OR REPLACE. Does NOT drop existing.       ║
-- ║  HOW TO RUN: Supabase Dashboard → SQL Editor → Paste → Run                  ║
-- ╚════════════════════════════════════════════════════════════════════════════════╝

-- ═══════════════════════════════════════════════════════════════════════════════
-- 1. NOTIFICATIONS (HR notification queue)
-- ═══════════════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS hrms_notifications (
  ...
  recipient_role TEXT,                    -- broadcast to role (e.g. 'hr_admin')
  ...
  -- Action
  action_url TEXT,                        -- deep link within HRMS
  action_label TEXT,
  ...
  -- Source
  source_module TEXT,                     -- 'leave', 'attendance', 'payroll', etc.
  source_entity_id UUID,
  
  -- Delivery channels (for future integration)
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- 2. WORKFORCE ANALYTICS VIEW (Aggregated employee statistics)
-- ═══════════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- 5. ATTRITION VIEW (Monthly exits for trend analysis)
-- ═══════════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- 6. HMS INTEGRATION VIEWS (Operational data → HR KPIs)
-- ═══════════════════════════════════════════════════════════════════════════════

-- Doctor Activity Summary (from HMS appointments/consultations if available)
-- This view provides aggregated data that can feed into Doctor KPIs
-- It uses hms_staff + existing HMS operational data
...
SELECT
  ...
  -- These would be populated from HMS operational tables when they have data
  0 AS consultations_today,
  0 AS consultations_month,
  0 AS revenue_month,
  0 AS follow_up_rate,
  0 AS avg_patient_rating
FROM hms_staff s
...
SELECT
  ...
  s.productivity_score,
  s.today_attendance,
  0 AS procedures_today,
  0 AS procedures_month,
  0 AS avg_therapy_rating
FROM hms_staff s
WHERE s.is_active = true
  AND (s.role ILIKE '%therapist%' OR s.department = 'Panchakarma')
ORDER BY s.name;

-- ═══════════════════════════════════════════════════════════════════════════════
-- 7. ALERTS GENERATION HELPER (Identifies conditions needing notifications)
-- ═══════════════════════════════════════════════════════════════════════════════
...
-- Qualifications/registrations expiring in next 60 days
...
SELECT
  ...
  q.registration_authority,
  q.registration_expiry,
...
-- Training certificates expiring in next 30 days
CREATE OR REPLACE VIEW hrms_v_expiring_training_certs AS
SELECT
  et.id AS training_record_id,
  et.employee_id,
  s.name AS employee_name,
  s.employee_code,
  t.name AS training_name,
  et.certificate_expiry,
...
-- Views are accessible to authenticated users (RLS on underlying tables still applies)
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- DONE — Phase 6 Intelligence & Notifications Created Successfully
-- ═══════════════════════════════════════════════════════════════════════════════
