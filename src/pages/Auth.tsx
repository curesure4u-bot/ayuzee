...
const signupSchema = z.object({
  ...
  phone: z
    ...
    .regex(/^[0-9+\-\s()]*$/i, "Phone can only contain digits and + - ( )")
    .optional()
    .or(z.literal("")),
  email: emailSchema,
...
const Auth = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const parsed = mode === "signup"
      ? signupSchema.safeParse({ fullName, phone, email, password })
      ...
      if (mode === "signup") {
        const { error } = await supabase.auth.signUp({
          ...
          options: {
            ...
            data: { full_name: fullName.trim(), phone: phone.trim(), ref_code: refCode },
          },
        });
