...
const Therapies = () => {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error: err } = await supabase
      ...
      .eq("is_active", true)
      .order("price");
    if (err) setError("Couldn't load therapies. Please try again.");
    else setItems((data ?? []) as Therapy[]);
    setLoading(false);
  }, []);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main>
        ...
        <section className="container py-12">
          {loading ? (
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3" aria-busy="true" aria-label="Loading therapies">
            ...
            <Card className="p-12 text-center">
              <Sparkles className="mx-auto h-12 w-12 text-destructive" />
              <p className="mt-3 font-medium">{error}</p>
              <Button variant="hero" className="mt-5" onClick={load}>Try again</Button>
            </Card>
          ) : items.length === 0 ? (
            <Card className="p-12 text-center">
