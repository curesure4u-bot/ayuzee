...
import { Loader2, X, AlertTriangle, CheckCircle2, Circle, ExternalLink } from "lucide-react";
            ...
            <Card className="p-4 space-y-2 text-sm">
              ...
              {c.medical_report_urls?.length > 0 && (
                <div className="mt-3 space-y-3">
                  {c.medical_report_urls.map((entry: string, i: number) => {
                    const isLegacyUrl = /^https?:\/\//i.test(entry);
                    const path = isLegacyUrl ? entry.split("/prescriptions/").pop() || "" : entry;
                    const openSigned = async () => {
                      if (isLegacyUrl && !path) {
                        window.open(entry, "_blank", "noreferrer");
                        return;
                      }
                      const { data, error } = await supabase.storage
                        .from("prescriptions")
                        .createSignedUrl(path, 900);
                      if (error || !data?.signedUrl) return toast.error(error?.message || "Could not open report");
                      window.open(data.signedUrl, "_blank", "noreferrer");
                    };
                    const fileName = path.split("/").pop() || `Report ${i + 1}`;
                    return (
                      <div key={i} className="rounded-lg border border-border p-2.5">
                        <div className="flex flex-wrap items-center gap-2">
                          <Button size="sm" variant="outline" onClick={openSigned}>
                            📄 Report {i + 1} <ExternalLink className="ml-1.5 h-3 w-3" />
                          </Button>
                          {path && (
                            <ReportSummaryButton
                              ...
                              path={path}
                              fileName={fileName}
                              refTable="atmri_sponsored_cases"
                              refId={c.id}
                              documentKind="patient-submitted medical report"
                              variant="inline"
                            />
                          )}
                        </div>
