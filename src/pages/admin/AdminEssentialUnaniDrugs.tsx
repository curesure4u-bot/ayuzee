...
const blank = {
  name: "", category: "Jawarishat", indications_text: "",
...
const AdminEssentialUnaniDrugs = () => {
  ...
  const load = async () => {
    const { data } = await (supabase as any).from("essential_unani_drugs").select("*").order("category").order("name");
    setDrugs(data ?? []);
  };
  useEffect(() => { load(); }, []);
  ...
  const save = async () => {
    ...
    if (editing) {
      const { error } = await (supabase as any).from("essential_unani_drugs").update(payload).eq("id", editing.id);
      if (error) return toast.error(error.message);
      toast.success("Updated");
    } else {
      payload.slug = `${slugify(form.name)}-${Date.now().toString(36)}`;
      const { error } = await (supabase as any).from("essential_unani_drugs").insert(payload);
      if (error) return toast.error(error.message);
  ...
  const remove = async (d: any) => {
    ...
    const { error } = await (supabase as any).from("essential_unani_drugs").delete().eq("id", d.id);
    if (error) return toast.error(error.message);
    ...
    <div className="space-y-4 p-4">
      <Card className="p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="font-display text-xl">Essential Unani Drugs</h1>
            <p className="text-xs text-muted-foreground">{drugs.length} formulations · AYUSH National Formulary</p>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div><Label>Indications (comma-separated)</Label>
              <Input value={form.indications_text} onChange={(e) => setForm({ ...form, indications_text: e.target.value })}
                placeholder="Nazla, Suda, Qabz" /></div>
            <div className="grid grid-cols-2 gap-3">
...
export default AdminEssentialUnaniDrugs;
