...
const AdminLearning = () => {
  ...
  const loadAll = async () => {
    const webinarListFields = "id, title, description, speaker_name, speaker_bio, speaker_avatar_url, cover_image_url, scheduled_at, duration_minutes, category, rsvp_count, is_published, created_at, updated_at";
    const [c, w] = await Promise.all([
      supabase.from("lms_courses").select("*").order("created_at", { ascending: false }),
      supabase.from("webinars").select(webinarListFields).order("scheduled_at", { ascending: false }),
    ]);
    setCourses(c.data ?? []); setWebinars(w.data ?? []);
  };

  const openWebinarEditor = async (webinar: any) => {
    if (!webinar?.id) { setEditingWebinar(webinar); return; }
    const { data, error } = await supabase.rpc("get_webinar_admin", { _webinar_id: webinar.id });
    if (error) { toast.error(error.message); return; }
    setEditingWebinar(data ?? webinar);
  };
                ...
                <div className="mt-4 space-y-3">
                  {webinars.map((w) => (
                    <Card key={w.id} className="flex items-center gap-4 p-4">
                      ...
                      <Button size="sm" variant="ghost" onClick={() => openWebinarEditor(w)}><Pencil className="h-4 w-4" /></Button>
