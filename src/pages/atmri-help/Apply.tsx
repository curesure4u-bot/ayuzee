...
const AtmriApply = () => {
  ...
  const uploadReports = async (): Promise<string[]> => {
    ...
    // Store storage PATHS (not public URLs) — the `prescriptions` bucket is private,
    // so viewers must sign the path on read. Backwards-compat: legacy rows may still
    // contain full https URLs; AdminAtmriHelp handles both.
    const paths: string[] = [];
