...
const EssentialUnaniDrugDetail = () => {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await (supabase as any)
        .from("essential_unani_drugs").select("*").eq("slug", slug).maybeSingle();
      setDrug(data);
      setLoading(false);
    })();
  }, [slug]);
  ...
  if (!drug) return <div className="container mx-auto p-8">Not found. <Link className="text-primary underline" to="/essential-unani-drugs">Back</Link></div>;

  return (
    <div className="container mx-auto max-w-3xl px-4 py-8">
      <Link to="/essential-unani-drugs" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-primary">
      ...
      <Card className="mt-4 p-6">
        ...
        {drug.indications?.length > 0 && (
          <section className="mt-6">
            <h2 className="text-sm font-semibold text-muted-foreground">Indications (Amraz)</h2>
        ...
        {drug.dose && (
          <section className="mt-5">
            <h2 className="text-sm font-semibold text-muted-foreground">Dose (Miqdar-e-Khurak)</h2>
        ...
        <div className="mt-6 flex gap-2">
          <Button asChild><Link to="/vaidya/unani-prescription">Use in prescription</Link></Button>
          <Button variant="outline" asChild><Link to="/essential-unani-drugs">Browse more</Link></Button>
        </div>
      </Card>

      <p className="mt-4 text-center text-xs text-muted-foreground">
        Source: Ministry of AYUSH — National Formulary of Unani Medicine. For qualified Hakim use.
      </p>
    </div>
  );
};

export default EssentialUnaniDrugDetail;
