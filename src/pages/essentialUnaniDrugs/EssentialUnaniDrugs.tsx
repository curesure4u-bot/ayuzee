...
const EssentialUnaniDrugs = () => {
  ...
  return (
    <div className="container mx-auto max-w-6xl px-4 py-8">
      <div className="mb-6">
        <h1 className="font-display text-3xl">AYUSH Essential Unani Drugs</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          National Formulary of Unani Medicine — {drugs.length} classical formulations across {categories.length} categories.
        </p>
      </div>
      ...
      <Card className="p-4">
        <div className="relative">
          ...
          <Input className="pl-9" placeholder="Search by name or indication (e.g. nazla, suda, qabz, sual…)" value={q} onChange={(e) => setQ(e.target.value)} />
        </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 lg:grid-cols-3">
        ...
        {filtered.map((d) => (
          <Link key={d.id} to={`/essential-unani-drugs/${d.slug}`}>
            <Card className="h-full p-4 transition-all hover:border-primary hover:shadow-md">
              ...
              {d.indications && d.indications.length > 0 && (
                <p className="mt-2 line-clamp-2 text-xs text-muted-foreground">
                  <span className="font-medium">Amraz:</span> {d.indications.slice(0, 6).join(", ")}
                </p>
              )}
              {d.dose && <p className="mt-1 text-xs"><span className="font-medium">Miqdar:</span> {d.dose}</p>}
            </Card>
          </Link>
        ))}
      </div>
...
export default EssentialUnaniDrugs;
