...
const HealthConditionDetail = () => {
  ...
  usePageSEO({
    title: c?.name
      ? `${c.name} — Ayurveda Treatment, Symptoms & Remedies | Ayuzee`
      : "Health Condition — Ayuzee",
    description: c?.tagline
      ? `${c.name}: ${c.tagline}`.slice(0, 158)
      : "Explore Ayurvedic treatment, symptoms, causes and doctor-recommended remedies for common health conditions on Ayuzee.",
      ...
      ? [
          {
            ...
            url: `https://ayuzee.com/health-conditions/${slug}`,
          },
          {
            "@context": "https://schema.org",
            "@type": "BreadcrumbList",
            itemListElement: [
              ...
              { "@type": "ListItem", position: 2, name: "Health Conditions", item: "https://ayuzee.com/health-conditions" },
              { "@type": "ListItem", position: 3, name: c.name, item: `https://ayuzee.com/health-conditions/${slug}` },
            ],
          },
        ]
      : undefined,
            ...
            <div className="grid gap-3">
              <div className="grid place-items-center rounded-2xl bg-background/70 p-4 shadow-soft">
                {c.hero_image_url || c.product_image_url ? (
                  <img src={c.hero_image_url ?? c.product_image_url ?? ""} alt={c.name} className="max-h-72 w-auto object-contain drop-shadow-xl"  loading="lazy" decoding="async" />
                ) : (
              ...
              {c.gallery_images.length > 0 && (
                <div className="flex flex-wrap items-center justify-center gap-2">
                  {c.gallery_images.slice(0, 6).map((src, i) => (
                    <img key={i} src={src} alt={`${c.name} ${i + 1}`} className="h-14 w-14 cursor-pointer rounded-md border border-border bg-background object-contain p-1 transition-smooth hover:border-primary"  loading="lazy" decoding="async" />
                  ))}
                </div>
              )}
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-3">
              {c.benefits.map((b, i) => (
                <div key={i} className="overflow-hidden rounded-2xl border border-border bg-background text-center">
                  {b.image_url && <img src={b.image_url} alt={b.title} className="h-40 w-full object-cover"  loading="lazy" decoding="async" />}
                  <div className="p-4">
            ...
            <div className="mt-6 grid gap-5 md:grid-cols-3">
              {c.how_it_works.map((s, i) => (
                <div key={i} className="rounded-2xl border border-border bg-background p-5 text-center">
                  {s.image_url && <img src={s.image_url} alt={s.title} className="mx-auto mb-3 h-28 object-contain"  loading="lazy" decoding="async" />}
              ...
              {c.doctor_feedback.map((d, i) => (
                <a key={i} href={d.video_url ?? "#"} target="_blank" rel="noreferrer" className="group overflow-hidden rounded-2xl border border-border bg-background">
                  <div className="aspect-[3/4] bg-muted">
                    {d.thumbnail_url && <img src={d.thumbnail_url} alt={d.doctor_name ?? "Doctor"} className="h-full w-full object-cover transition-transform group-hover:scale-105"  loading="lazy" decoding="async" />}
                  </div>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {c.patient_feedback.map((p, i) => (
                <a key={i} href={p.video_url ?? "#"} target="_blank" rel="noreferrer" className="group overflow-hidden rounded-2xl border border-border bg-background">
                  <div className="aspect-[3/4] bg-muted">
                    {p.thumbnail_url && <img src={p.thumbnail_url} alt={p.patient_name ?? "Patient"} className="h-full w-full object-cover transition-transform group-hover:scale-105"  loading="lazy" decoding="async" />}
                  </div>
                  <div className="p-3">
        ...
        {(c.approach_title || c.approach_body) && (
          <section className="container mt-12">
            <div className="grid items-center gap-6 rounded-3xl bg-background p-8 md:grid-cols-2">
              ...
              {c.approach_image_url && <img src={c.approach_image_url} alt="Approach" className="rounded-2xl"  loading="lazy" decoding="async" />}
            </div>
          </section>
        )}
          ...
          <section className="container mt-12">
            ...
            <div className="mt-6 grid items-center gap-6 rounded-3xl border border-border bg-background p-6 md:grid-cols-[2fr_1fr]">
              ...
              {c.approach_image_url && (
                <img src={c.approach_image_url} alt="Ayurveda" className="hidden h-full w-full rounded-2xl object-cover md:block"  loading="lazy" decoding="async" />
              )}
            </div>
          </section>
        )}
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-3 lg:grid-cols-6">
              {c.ingredients.map((ing, i) => (
                <div key={i} className="grid place-items-center rounded-2xl border border-border bg-background p-4 text-center">
                  {ing.image_url && <img src={ing.image_url} alt={ing.name} className="mb-2 h-20 w-20 object-contain"  loading="lazy" decoding="async" />}
          ...
          <section className="container mt-12">
            ...
            <div className="mt-6 grid items-center gap-6 rounded-3xl border border-border bg-background p-6 md:grid-cols-2">
              {c.how_to_use.image_url ? (
                <img src={c.how_to_use.image_url} alt="How to use" className="h-64 w-full rounded-2xl object-cover"  loading="lazy" decoding="async" />
              ) : (
              ...
              {c.videos.map((v, i) => (
                <a key={i} href={v.video_url ?? "#"} target="_blank" rel="noreferrer" className="group relative block overflow-hidden rounded-2xl border border-border bg-background">
                  <div className="aspect-video bg-muted">
                    {v.thumbnail_url && <img src={v.thumbnail_url} alt={v.title ?? "Video"} className="h-full w-full object-cover transition-transform group-hover:scale-105"  loading="lazy" decoding="async" />}
                  </div>
