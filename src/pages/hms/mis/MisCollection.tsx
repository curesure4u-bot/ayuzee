...
import {
  ...
  TrendingUp, Users, Building2, CreditCard, Wallet, Loader2
} from "lucide-react";
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, PieChart, Pie, Cell } from "recharts";
import { useMisCollection, type MisCollectionFilters } from "@/hooks/useMisCollection";
...
interface MisCollectionProps {
  dateFrom?: string;
  dateTo?: string;
  location?: string;
}

const MisCollection = ({ dateFrom, dateTo, location }: MisCollectionProps) => {
  const [activeReport, setActiveReport] = useState("daily-summary");

  // Use today's date as default if no props provided
  const today = new Date().toISOString().split("T")[0];
  const filters: MisCollectionFilters = {
    dateFrom: dateFrom || today,
    dateTo: dateTo || today,
    location: location || "all",
  };

  const { dailySummary, collectionByDept, incomeByPaymentType, hourlyCollection, grandTotal, loading, error } = useMisCollection(filters);
    ...
    <div className="space-y-4 mt-4">
        ...
        <CardContent className="p-3">
          <div className="flex items-start gap-2">
            ...
            <div className="text-xs text-muted-foreground">
              ...
              {loading ? "Analyzing collection data..." : (
                `Collection ₹${grandTotal.toLocaleString("en-IN")} today. ` +
                (incomeByPaymentType[0] ? `Top mode: ${incomeByPaymentType[0].name} (₹${incomeByPaymentType[0].value.toLocaleString("en-IN")}). ` : "") +
                (dailySummary[0] ? `${dailySummary[0].user} has highest collection. ` : "") +
                `Consider shifting more patients to digital for faster reconciliation.`
              )}
            </div>
          </div>
        </CardContent>
      ...
      {loading && (
        <div className="flex items-center justify-center py-4 text-muted-foreground gap-2">
          ...
          <span className="text-xs">Loading collection data...</span>
        </div>
      )}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                <tr className="bg-muted/50 font-bold">
                  ...
                  <td className="px-3 py-2 text-xs text-right">{dailySummary.reduce((s, r) => s + r.cash, 0).toFixed(2)}</td>
                  <td className="px-3 py-2 text-xs text-right">{dailySummary.reduce((s, r) => s + r.card, 0).toFixed(2)}</td>
                  <td className="px-3 py-2 text-xs text-right">{dailySummary.reduce((s, r) => s + r.cheque, 0).toFixed(2)}</td>
                  <td className="px-3 py-2 text-xs text-right">{dailySummary.reduce((s, r) => s + r.dd, 0).toFixed(2)}</td>
                  <td className="px-3 py-2 text-xs text-right">{dailySummary.reduce((s, r) => s + r.neft, 0).toFixed(2)}</td>
                  <td className="px-3 py-2 text-xs text-right">{dailySummary.reduce((s, r) => s + r.credit, 0).toFixed(2)}</td>
                  <td className="px-3 py-2 text-xs text-right text-green-600">{dailySummary.reduce((s, r) => s + r.gpay, 0).toFixed(2)}</td>
            ...
            <ResponsiveContainer width="100%" height={180}>
              <PieChart>
                <Pie data={incomeByPaymentType} cx="50%" cy="50%" outerRadius={65} dataKey="value"
                  ...
                  {incomeByPaymentType.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                </Pie>
