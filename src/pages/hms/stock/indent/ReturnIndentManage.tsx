...
const ReturnIndentManage = () => {
  const [records, setRecords] = useState<ReturnIndentRecord[]>([]);
  ...
  const loadRecords = async () => {
    ...
    try {
      const { data, error } = await (supabase as any)
        ...
        .ilike("transfer_reason", "%Return Indent%")
      ...
      toast.error("Failed to load return indents");
      console.error(err);
    }
    setLoading(false);
  };
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {records.length === 0 ? (
                  <tr><td colSpan={7} className="px-4 py-8 text-center text-muted-foreground">No return indent records found</td></tr>
                ) : (
                  records.map((r, idx) => (
