...
const CreditPatient = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (store !== "all") query = query.eq("ward_store_id", store);
      ...
      // Filter by patient name in notes if specified
      if (patient.trim()) {
        filtered = filtered.filter((r: any) => (r.notes || "").toLowerCase().includes(patient.toLowerCase()));
      }
  ...
  const totalCredit = records.reduce((s, r) => s + (r.bill_amount || 0), 0);
        ...
        <CardContent className="p-6">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div>
              ...
              <Input className="w-[150px]" placeholder="Patient name" value={patient} onChange={(e) => setPatient(e.target.value)} />
            </div>
            <div>
        ...
        <Card>
          <CardContent className="p-0">
            <div className="px-4 py-2 border-b bg-muted/30 text-sm font-semibold">
              {records.length} records | Total: ₹{totalCredit.toLocaleString()}
            </div>
