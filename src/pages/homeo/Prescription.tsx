...
const REPETITIONS = ["Single dose", "BD x 3 days", "TDS x 5 days", "OD x 7 days", "Weekly x 4", "Monthly x 3", "SOS"];
...
const Prescription = () => {
  ...
  const save = async () => {
    if (!caseId) return toast.error("Open from a case to save prescription");
    ...
    setList([{ remedy_id: "", remedy_name: "", potency: "30C", repetition: "Single dose", instructions: "" }]);
    const { data: rx } = await supabase.from("homeo_prescriptions").select("*").eq("case_id", caseId).order("prescribed_at", { ascending: false });
    setHistory(rx ?? []);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`${t.card} p-5 space-y-4`}>
        <div>
          <label className={t.label}>Diagnosis / Working hypothesis</label>
          <input className={`${t.input} mt-1`} value={diagnosis} onChange={(e) => setDiagnosis(e.target.value)} placeholder="e.g. Acute coryza, Bryonia state" />
        </div>
          ...
          <div className="mt-2 space-y-3">
            {list.map((row, i) => (
              <div key={i} className="grid gap-2 md:grid-cols-12">
                ...
                <select className={`${t.input} md:col-span-3`} value={row.repetition} onChange={(e) => updateRow(i, "repetition", e.target.value)}>
                  {REPETITIONS.map((p) => <option key={p}>{p}</option>)}
                </select>
            ...
            <button onClick={() => setList([...list, { remedy_id: "", remedy_name: "", potency: "30C", repetition: "Single dose", instructions: "" }])} className={t.ghostBtn}>+ Add row</button>
          </div>
        </div>

        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <label className={t.label}>General advice</label>
            <textarea className={`${t.input} mt-1 min-h-[80px]`} value={advice} onChange={(e) => setAdvice(e.target.value)} placeholder="Diet, lifestyle, dos and don'ts" />
          </div>
          <div>
          ...
          <ul className="divide-y divide-[hsl(45_40%_55%/0.12)]">
            {history.map((h) => (
              <li key={h.id} className="py-3 text-sm">
                <span className={t.goldText}>{h.remedy_name}</span> · {h.potency} · {h.repetition}
