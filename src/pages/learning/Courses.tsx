        ...
        <Link key={c.id} to={`/learning/courses/${c.slug}`} className="group">
          <Card className="overflow-hidden transition-smooth hover:-translate-y-1 hover:shadow-elegant">
            <div className="aspect-video overflow-hidden gradient-soft">
              {c.thumbnail_url ? <img src={c.thumbnail_url} alt={c.title} className="h-full w-full object-cover transition-smooth group-hover:scale-105" /> : <div className="grid h-full place-items-center"><PlayCircle className="h-16 w-16 text-primary/30" /></div>}
            </div>
            <div className="p-5">
