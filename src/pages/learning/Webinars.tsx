...
const Webinars = () => {
  ...
  const [accessLinks, setAccessLinks] = useState<Record<string, { join_url: string | null; recording_url: string | null }>>({});
  ...
  const load = async () => {
    const { data: w } = await supabase.from("webinars").select(PUBLIC_FIELDS).eq("is_published", true).order("scheduled_at", { ascending: true });
    setWebinars(w ?? []);
    ...
    if (uid) {
      ...
      // Fetch access links (join_url / recording_url) via secured RPC
      const links: Record<string, { join_url: string | null; recording_url: string | null }> = {};
      await Promise.all((w ?? []).map(async (item: any) => {
        const { data: acc } = await supabase.rpc("get_webinar_access", { _webinar_id: item.id });
        const row = Array.isArray(acc) ? acc[0] : acc;
        links[item.id] = { join_url: row?.join_url ?? null, recording_url: row?.recording_url ?? null };
      }));
  ...
  return (
    <div className="grid gap-5 md:grid-cols-2">
      {webinars.map((w) => {
        ...
        const access = accessLinks[w.id];
        const recordingUrl = access?.recording_url ?? null;
        const joinUrl = access?.join_url ?? null;
        return (
          <Card key={w.id} className="overflow-hidden">
            {w.cover_image_url && <img src={w.cover_image_url} alt={w.title} className="aspect-video w-full object-cover"  loading="lazy" decoding="async" />}
            <div className="p-5">
              ...
              <div className="mt-5 flex gap-2">
                {isPast && recordingUrl ? (
                  <Button asChild variant="hero" className="flex-1"><a href={recordingUrl} target="_blank" rel="noopener noreferrer">Watch replay <ExternalLink className="h-3.5 w-3.5" /></a></Button>
                ) : isPast && !userId ? (
                  <Button onClick={() => navigate("/auth")} variant="outline" className="flex-1">Sign in to watch replay</Button>
                ) : isPast ? (
                  ...
                  <>
                    ...
                    {registered && joinUrl && <Button asChild variant="hero" className="flex-1"><a href={joinUrl} target="_blank" rel="noopener noreferrer">Join <ExternalLink className="h-3.5 w-3.5" /></a></Button>}
                  </>
                )}
              </div>
            </div>
