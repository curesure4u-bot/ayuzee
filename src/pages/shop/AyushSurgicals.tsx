...
const SURGICAL_CATEGORIES = [
  "🔪 Ksharasutra Kits",
  "🔥 Agnikarma Instruments",
  "🩹 Wound Care & Bandaging",
  "🧤 Disposables & PPE",
  "🔬 Diagnostic Instruments",
  "💉 Panchakarma Disposables",
  "🏥 OT Accessories",
];
...
const AyushSurgicals = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter((p) => `${p.surgical_category ?? ""} ${p.category} ${p.name}`.toLowerCase().includes(needle));
  }, [category, products]);
      ...
      <section className="border-b border-border bg-background">
        <div className="container py-10 md:py-14">
          ...
          <div className="mt-6 rounded-xl border border-border bg-primary/10 p-4 text-sm font-medium text-foreground">
            🔒 Some surgical items require a doctor/therapist account to purchase.
          </div>
        </div>
      </section>
      ...
      <main className="container py-6">
        <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
          <aside className="rounded-xl border border-border bg-background p-4 lg:sticky lg:top-24 lg:self-start">
            ...
            <button onClick={() => setCategory("All")} className={cn("mb-2 block w-full rounded-md px-3 py-2 text-left text-sm transition-smooth", category === "All" ? "bg-primary text-primary-foreground" : "hover:bg-accent")}>All surgicals</button>
          ...
          <section>
            <h2 className="mb-5 font-display text-2xl">
              {category === "All" ? "All Surgicals" : category} <span className="text-sm font-normal text-muted-foreground">- {filtered.length} items</span>
            </h2>
            ...
            {loading ? (
              ...
              <div className="rounded-xl border border-dashed border-border bg-background p-12 text-center">
                <Stethoscope className="mx-auto h-14 w-14 text-primary/30" />
                <p className="mt-4 font-semibold">No surgical products available yet</p>
                <p className="mt-1 text-sm text-muted-foreground">Our procurement team is verifying suppliers — check back soon.</p>
              </div>
            ) : (
                ...
                {filtered.map((p) => (
                  <article key={p.id} className="group relative flex flex-col rounded-xl border border-border bg-background p-3 transition-smooth hover:shadow-soft">
                    ...
                    <div className="mb-2 flex flex-wrap items-center gap-1">
                      {p.surgical_category && <Badge variant="secondary" className="rounded-full text-[10px]">{p.surgical_category}</Badge>}
                      {p.is_prescription_required && <Badge variant="outline" className="rounded-full text-[10px]"><ShieldAlert className="mr-1 h-3 w-3" /> Pro only</Badge>}
                    ...
                    <Button className="mt-3 w-full rounded-full" disabled={p.stock <= 0} onClick={() => addToCart(p)}>
