...
type AccessLinks = { join_url: string | null; recording_url: string | null };

const PUBLIC_FIELDS = "id, title, description, speaker_name, speaker_bio, speaker_avatar_url, cover_image_url, scheduled_at, duration_minutes, category, rsvp_count, is_published";
...
const StudentWebinars = () => {
  ...
  const [accessLinks, setAccessLinks] = useState<Record<string, AccessLinks>>({});
  const now = useMemo(() => new Date(), [loading]);

  useEffect(() => { load(); }, []);

  const fetchAccess = async (ids: string[]) => {
    const map: Record<string, AccessLinks> = {};
    await Promise.all(ids.map(async (id) => {
      const { data } = await supabase.rpc("get_webinar_access", { _webinar_id: id });
      const row = Array.isArray(data) ? data[0] : data;
      map[id] = { join_url: row?.join_url ?? null, recording_url: row?.recording_url ?? null };
    }));
    return map;
  };
  ...
  const load = async () => {
    ...
    const [upcomingRes, recordingRes, rsvpRes] = await Promise.all([
      supabase.from("webinars").select(PUBLIC_FIELDS).eq("is_published", true).gt("scheduled_at", new Date().toISOString()).order("scheduled_at", { ascending: true }),
      supabase.from("webinars").select(PUBLIC_FIELDS).eq("is_published", true).lt("scheduled_at", new Date().toISOString()).order("scheduled_at", { ascending: false }),
      uid ? supabase.from("webinar_rsvps").select(`id, webinar_id, webinars(${PUBLIC_FIELDS})`).eq("user_id", uid).order("created_at", { ascending: false }) : Promise.resolve({ data: [] }),
    ]);

    const upcomingList = (upcomingRes.data ?? []) as Webinar[];
    const recordingList = (recordingRes.data ?? []) as Webinar[];
    const rsvpList = rsvpRes.data ?? [];

    setUpcoming(upcomingList);
    setRsvps(rsvpList);
    setRsvpIds(new Set(rsvpList.map((item: any) => item.webinar_id)));

    if (uid) {
      const allIds = Array.from(new Set([
        ...upcomingList.map((w) => w.id),
        ...recordingList.map((w) => w.id),
        ...rsvpList.map((r: any) => r.webinar_id),
      ]));
      const links = await fetchAccess(allIds);
      setAccessLinks(links);
      // Only include recordings whose recording_url is visible to this user
      setRecordings(recordingList.filter((w) => links[w.id]?.recording_url));
    } else {
      setRecordings([]);
      setAccessLinks({});
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="upcoming" className="space-y-5">
        ...
        <TabsContent value="rsvps">{rsvps.length === 0 ? <Empty text="Your webinar RSVPs will appear here." /> : <div className="grid gap-5 lg:grid-cols-2">{rsvps.map((item) => <RsvpCard key={item.id} webinar={item.webinars} joinUrl={accessLinks[item.webinar_id]?.join_url ?? null} />)}</div>}</TabsContent>
        <TabsContent value="recordings"><WebinarGrid webinars={recordings} action={(webinar) => {
          const url = accessLinks[webinar.id]?.recording_url;
          return url ? <Button asChild><a href={url} target="_blank" rel="noopener noreferrer">Watch Recording <ExternalLink className="h-4 w-4" /></a></Button> : <Button disabled variant="outline">Recording unavailable</Button>;
        }} now={now} /></TabsContent>
      </Tabs>
    </div>
  );
...
const RsvpCard = ({ webinar, joinUrl }: { webinar: Webinar; joinUrl: string | null }) => {
  if (!webinar) return null;
  const start = new Date(webinar.scheduled_at);
  const live = Math.abs(start.getTime() - Date.now()) <= 15 * 60 * 1000;
  return <Card><CardContent className="p-5"><Badge variant="outline">RSVP Confirmed</Badge><h3 className="mt-3 font-display text-xl">{webinar.title}</h3><p className="mt-2 text-sm text-muted-foreground">{start.toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })}</p><Button asChild disabled={!live || !joinUrl} className="mt-5"><a href={joinUrl || "#"} target="_blank" rel="noopener noreferrer">{live ? "Join Live" : "Join link opens near start"}</a></Button></CardContent></Card>;
};
