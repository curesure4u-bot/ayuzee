...
const BillsPage = ({ type }: Props) => {
  ...
  const downloadPDF = () => {
    ...
    const isInter = !!previewBill.is_interstate;
    const gstRate = Number(previewBill.gst_rate || 0);
    const { doc } = startAyuzeePdf({
      clinicName: previewBill.clinic_name || "Ayuzee Clinic",
      subtitle: type === "patient_bill" ? "Tax Invoice" : "Direct Sale Invoice",
    });
    let y = addTitle(doc, 38, `Invoice ${previewBill.bill_no || ""}`, new Date(previewBill.bill_date).toLocaleDateString("en-IN"));
    y = addPlainTable(doc, y, [
      ["Clinic", previewBill.clinic_name || "—"],
      ["Address", previewBill.clinic_address || "—"],
      ["GSTIN", previewBill.gstin || "—"],
      ["Patient", previewBill.patient_name],
      ["Phone", previewBill.patient_phone || "—"],
      ["Patient GSTIN", previewBill.patient_gstin || "—"],
      ["Payment", `${String(previewBill.payment_mode || "").toUpperCase()}${previewBill.payment_reference ? ` (${previewBill.payment_reference})` : ""}`],
      ["Status", String(previewBill.status || "").toUpperCase()],
    ]);
    y = addSectionTable(doc, y, {
      title: "Items",
      head: ["#", "Description", "Type", "HSN/SAC", "Qty", "Rate", "Amount"],
      body: previewItems.map((i, idx) => [
        idx + 1,
        i.medicine_name,
        i.item_type || "medicine",
        i.hsn_code || "—",
        i.quantity,
        fmtINR(i.unit_price),
        fmtINR(i.line_total),
    ...
    const totalsBody: [string, string][] = [
      ["Subtotal", fmtINR(Number(previewBill.subtotal))],
    ];
    if (Number(previewBill.discount) > 0) totalsBody.push(["Discount", `− ${fmtINR(Number(previewBill.discount))}`]);
    if (Number(previewBill.gst_amount) > 0) {
      if (isInter) {
        totalsBody.push([`IGST (${gstRate}%)`, fmtINR(Number(previewBill.igst_amount))]);
      } else {
        totalsBody.push([`CGST (${gstRate / 2}%)`, fmtINR(Number(previewBill.cgst_amount))]);
        totalsBody.push([`SGST (${gstRate / 2}%)`, fmtINR(Number(previewBill.sgst_amount))]);
      }
    }
    totalsBody.push(["Grand Total", fmtINR(Number(previewBill.total))]);
    y = addSectionTable(doc, y, {
      title: "Totals",
      body: totalsBody,
    ...
    finalizeAyuzeePdf(
      doc,
      `Invoice-${safeFileName(previewBill.bill_no || previewBill.patient_name)}.pdf`,
