...
const HomeopathyPrescription = () => {
  ...
  const downloadPDF = () => {
    ...
    const { doc } = startAyuzeePdf({
      clinicName: "Ayuzee Homeopathy Clinic",
      subtitle: "Homeopathy Prescription",
    });
    let y = addTitle(doc, 38, "Prescription", new Date().toLocaleDateString());
    ...
    y = addSectionTable(doc, y, {
      title: "Rx",
      head: ["#", "Remedy", "Kingdom", "Potency", "Dose", "Frequency", "Repetition", "Instructions"],
      body: lines.map((l, i) => [
        i + 1, l.drug.name, l.drug.kingdom, l.potency, l.dose, l.frequency, l.repetition || "—", l.instructions || "",
      ]),
      columnStyles: { 0: { cellWidth: 8 } },
    });
    finalizeAyuzeePdf(
      doc,
      `Homeopathy-Rx-${safeFileName(patientName)}-${Date.now()}.pdf`,
