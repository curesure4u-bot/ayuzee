...
const SiddhaPrescription = () => {
  ...
  const downloadPDF = () => {
    ...
    const { doc } = startAyuzeePdf({
      clinicName: "Ayuzee Siddha Clinic",
      subtitle: "Siddha Prescription",
    });
    ...
    y = addPlainTable(doc, y, [
      ...
      ["Diagnosis (Noi Naadal)", diagnosis || "—"],
    ...
    if (advice.trim()) y = addParagraph(doc, y, "Pathiyam / Lifestyle advice", advice);
    finalizeAyuzeePdf(
      doc,
      `Siddha-Rx-${safeFileName(selectedPatient?.full_name)}-${Date.now()}.pdf`,
