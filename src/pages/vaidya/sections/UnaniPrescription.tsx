...
type Line = {
  drug: Drug; dose_override: string; frequency: string;
  duration: string; anupana: string; instructions: string;
};

const FREQ = ["OD (once a day)", "BD (twice)", "TID (thrice)", "QID (4 times)", "HS (bedtime)", "SOS"];
const ANUPANA = ["Aab-e-Garm (Warm water)", "Shahad (Honey)", "Sheer (Milk)", "Roghan-e-Zard (Ghee)", "Lassi", "Arq-e-Gulab", "Sharbat-e-Bazoori"];

const UnaniPrescription = () => {
  ...
  const addDrug = (d: Drug) => {
    if (lines.find((l) => l.drug.id === d.id)) { toast.info("Already in prescription"); return; }
    setLines((prev) => [...prev, {
      drug: d, dose_override: d.dose ?? "", frequency: "BD (twice)",
      duration: "7 days", anupana: "Aab-e-Garm (Warm water)", instructions: "",
    }]);
    setSearch("");
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      const { error: pErr } = await (supabase as any).from("prescription_essential_unani_drugs").insert(rows);
      if (pErr) throw pErr;
  ...
  const downloadPDF = () => {
    ...
    const { doc } = startAyuzeePdf({
      clinicName: "Ayuzee Unani Clinic",
      subtitle: "Unani Prescription (Nuskha)",
    });
    let y = addTitle(doc, 38, "Nuskha (Prescription)", new Date(visitDate).toLocaleDateString());
    y = addPlainTable(doc, y, [
      ...
      ["Tashkhees (Diagnosis)", diagnosis || "—"],
      ["Follow-up", followUp ? new Date(followUp).toLocaleDateString() : "—"],
    ]);
    y = addSectionTable(doc, y, {
      title: "Nuskha",
      head: ["#", "Dawa", "Miqdar", "Daf'aat", "Muddat", "Anupana", "Hidayat"],
      body: lines.map((l, i) => [
        i + 1, l.drug.name, l.dose_override, l.frequency, l.duration, l.anupana, l.instructions || "",
      ]),
      columnStyles: { 0: { cellWidth: 8 } },
    });
    if (advice.trim()) y = addParagraph(doc, y, "Ilaj-bil-Tadbeer (Lifestyle advice)", advice);
    finalizeAyuzeePdf(
      doc,
      `Unani-Rx-${safeFileName(selectedPatient?.full_name)}-${Date.now()}.pdf`,
    ...
    <div className="mx-auto max-w-5xl space-y-4 p-4">
      ...
      <Card className="p-4 print:hidden">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="font-display text-xl">Unani Prescription Writer</h1>
            <p className="text-xs text-muted-foreground">
              Powered by AYUSH Essential Unani Drugs ({allDrugs.length} formulations) — auto-fills miqdar (dose), anupana, ihtiyatat (precautions).
            </p>
          </div>
        </div>
      </Card>
      ...
      <Card className="p-4 print:shadow-none print:border-0">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          ...
          <div><Label>Visit date</Label><Input type="date" value={visitDate} onChange={(e) => setVisitDate(e.target.value)} /></div>
          <div><Label>Follow-up</Label><Input type="date" value={followUp} onChange={(e) => setFollowUp(e.target.value)} /></div>
        </div>
        <div className="mt-3">
          <Label>Diagnosis (Tashkhees)</Label>
          <Input value={diagnosis} onChange={(e) => setDiagnosis(e.target.value)} placeholder="e.g. Nazla-e-Haar (acute coryza)" />
        </div>
      </Card>
      ...
      <Card className="p-4 print:hidden">
        <div className="mb-3 flex items-center justify-between gap-2">
          ...
          <a href="/essential-unani-drugs" target="_blank" rel="noopener noreferrer"
            ...
            📋 View Unani Drug Reference ↗
          </a>
        </div>
        <Label>Search & add drug</Label>
        <div className="relative mt-1">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <Input className="pl-9" placeholder="Type name or indication: jawarish, nazla, qabz, sual…"
            ...
            {lines.map((l, i) => (
              <div key={l.drug.id} className="rounded-lg border border-border p-3">
                ...
                <div className="mt-2 grid grid-cols-2 gap-2 md:grid-cols-4 print:hidden">
                  <div><Label className="text-xs">Dose (Miqdar)</Label>
                    <Input value={l.dose_override} onChange={(e) => updateLine(i, { dose_override: e.target.value })} /></div>
                  ...
                  <div><Label className="text-xs">Duration</Label>
                    <Input value={l.duration} onChange={(e) => updateLine(i, { duration: e.target.value })} /></div>
                  <div><Label className="text-xs">Anupana (Vehicle)</Label>
                    <select className="w-full rounded-md border border-input bg-background px-2 py-2 text-sm"
                ...
                <div className="mt-2 print:hidden">
                  ...
                  <Input value={l.instructions} onChange={(e) => updateLine(i, { instructions: e.target.value })} placeholder="e.g. after food" />
                </div>
      ...
      <Card className="p-4 print:shadow-none print:border-0">
        <Label>Parhez / lifestyle advice</Label>
        <Textarea rows={3} value={advice} onChange={(e) => setAdvice(e.target.value)}
          placeholder="Diet (ghiza), regimen (tadbeer), do's & don'ts (parhez)…" />
      </Card>
...
export default UnaniPrescription;
