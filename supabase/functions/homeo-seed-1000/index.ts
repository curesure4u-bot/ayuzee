...
async function callAI(LOVABLE_API_KEY: string, chapter: string, subcats: string[], count: number, knownAbbrs: string[]) {
  const resp = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: "user",
        content: `Generate exactly ${count} classical homeopathy rubrics for chapter "${chapter}".
...
Return ONLY via the tool call.`,
      ...
      tool_choice: { type: "function", function: { name: "emit_rubrics" } },
    }),
  });
  ...
  return (args.rubrics ?? []) as RubricRow[];
}
  ...
  try {
    ...
    for (const cat of CATEGORY_PLAN) {
      ...
      while (remaining > 0) {
        ...
        try {
          rubrics = await callAI(LOVABLE_API_KEY, cat.chapter, cat.subcats, batch, knownAbbrs);
        } catch (e) {
