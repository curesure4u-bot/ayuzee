...
async function aiGenerate(name: string, abbr: string, authHeader: string): Promise<DetailedRemedy | null> {
  ...
  return (data.result as DetailedRemedy | undefined) ?? null;
}
...
Deno.serve(async (req) => {
  ...
  try {
    // Admin-only: this function performs expensive AI calls and overwrites
    // production homeo_remedies rows. Require a valid JWT + admin role.
    ...
    const { data: isAdmin } = await authClient.rpc("has_role", {
      _user_id: udata.user.id, _role: "admin",
    });
    if (!isAdmin) {
    ...
    for (const r of TOP_200) {
      ...
      if (DETAILED_NAMES.has(r.name)) {
        const detail = await aiGenerate(r.name, r.abbr, authHeader);
