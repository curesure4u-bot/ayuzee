...
serve(async (req) => {
  ...
  try {
    ...
    // 2) Ask Lovable AI to expand to ~200 more remedies + generate symptom rubrics
    const aiResp = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: "user",
          content: `Generate a JSON object with two arrays for a classical homeopathy seed database.
...
Return ONLY valid JSON, no prose.`,
        ...
        tool_choice: { type: "function", function: { name: "seed_homeo" } },
      }),
    });
